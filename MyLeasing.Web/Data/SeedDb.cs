...
namespace MyLeasing.Web.Data
{
    public class SeedDb
    {
        ...
        {
            await _context.Database.EnsureCreatedAsync(); //Valida que al pasar en ese punto DB esté creada
                                                          //await se porque la generacion de la  DB puede llevaste mucho tiempo
            await CheckPropertyTypesAsync(); //Método para crear los tipo de propiedad, funciona iguaul para los otros
        ...
        {
            if (!_context.PropertyTypes.Any()) //Valida si hay tipos de propiedad; 'Any()' para verificar si hay al menos uno
            {
                ...
                await _context.SaveChangesAsync(); // este guarda los datos en la Db
            }
        }
        ...
        {
            var owner = _context.Owners.FirstOrDefault(); //.FirstOrDefault() este busca el primer propietaio ingresado
            var propertyType = _context.PropertyTypes.FirstOrDefault();
            if (!_context.Properties.Any())
            {
                AddProperty("Calle 43 #23 32", "Poblado", owner, propertyType, 800000M, 2, 72, 4); //las constantes decimales se les agrega una N, sino la trata como double
